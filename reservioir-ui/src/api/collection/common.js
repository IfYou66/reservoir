...
export const SENSOR_URL = '/second-api/getSensor/serverSensor.ashx';  //10304
export const SENSOR_URL_NEW = '/first-api/getSensor/serverSensor.ashx'; //10301
  ...
  for (let i in sensors) {
    for (let j in newData) {
      if (isSame(sensors[i], newData[j])) {
        if (sensors[i].tag === 'dam-accumulate-down-speed') {
        }
...
function isSame(sensor, sen) {
  if (sensor.id && sen.id) {
    return sensor.id === sen.id;
  } else if (sensor.tag && sen.tag) {
    return sensor.tag === sen.tag;
  } else {
    return sensor.part === sen.part && sensor.section === sen.section && sensor.name === sen.name && sensor.type === sen.type && sensor.title === sensor.title;
  }
}
...
export function correct(value, bias) {
  if (value === common.INVALID ||
    Math.abs(value) === common.BOUNDARY) {
    return value;
  }
...
//校正一个值，负数变成一个小的正值
export function makePositive(value) {
    return (value > 0 ? value : (Math.random() * 0.03))
}

/**
 * 去掉对象数组中的重复项
 * @param arr 数组
 * @param attribute 重复项查找属性
 * @returns {*} 去重之后的数组
 */
export function removeRepeat(arr, attribute) {
  let hash = {};
  let newData = [...arr];
  return newData.reduce(function (item, next) {
    hash[next[attribute]] ? '' : hash[next[attribute]] = item.push(next);
    return item;
  }, [])
}
